...
def load_langpair_dataset(
    ...
    print("shape: ", src_dataset[0].shape)
    len_shape = src_dataset[0].shape[0]
    index = np.where(src_dataset[0].numpy() == 2)
    print("index: ", index)
    # slit dataset into 2 parts: key_answer + question
...
class AutoEncodeingRegressiveTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--max_source_positions', default=1024, type=int, metavar='N',
                            help='max number of tokens in the source sequence')
        parser.add_argument('--max_target_positions', default=1024, type=int, metavar='N',
        ...
        # extend from papers
        parser.add_argument('--decoder_layerdrop', default=0, type=float, 
                            help='decoder layer dropout')
        parser.add_argument('--quant_noise_pq', default=0, type=int, 
                            help='Noise')
        parser.add_argument('--no_token_positional_embeddings', default=True, 
                            help='Token positional_embedding')
        ############################################################
    ...
    def setup_task(cls, args, **kwargs):
        ...
        src_dict = cls.load_dictionary(
            os.path.join(str(paths[0]), "dict.{}.txt".format(args.source_lang))
        )
        tgt_dict = cls.load_dictionary(
            os.path.join(str(paths[0]), "dict.{}.txt".format(args.target_lang))
        )
